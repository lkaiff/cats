...
class BotFarmer(BaseFarmer):
    name = 'catsgang_bot'
    extra_code = "viYPTdmEts4qTwI6D4Exk"
    ...
    def user(self):
        response = self.scraper.get(URL_USER)
    ...
    def tasks(self):
        ...
        try:
            response = self.scraper.get(URL_GTASK).json()
            ...
            for task in tasks:
                ...
                if not completed and task_id not in skip_ids:
                    ...
                    if task_type not in skip_ids:
                        ...
                        result = self.scraper.post(f'{URL_CTASK}{task_id}/complete').json()
    ...
    def cats_avatar(self):
        try:
            avatar_info = self.scraper.get('https://cats-backend-cxblew-prod.up.railway.app/user/avatar').json()
            if avatar_info:
                ...
                if time_difference >= 24 * 3600:
                    ...
                    response = self.scraper.post('https://cats-backend-cxblew-prod.up.railway.app/user/avatar/upgrade', data=form_data, headers=headers).json()
